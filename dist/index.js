...
__export(src_exports, {
  ...
  generateFlowHtml: () => generateFlowHtml,
  test: () => test
});
...
.git-commit{color:var(--t2);font-family:var(--mono)}
.git-tag{color:#fbbf24;border-color:#fbbf2440}
...
function gitExec(cmd) {
  try {
    return (0, import_node_child_process.execSync)(cmd, { encoding: "utf-8", timeout: 5e3, stdio: ["pipe", "pipe", "pipe"] }).trim() || void 0;
  } catch {
    return void 0;
  }
}
function collectGitInfo() {
  return {
    ...
    repositoryName: gitExec("git remote get-url origin")?.replace(/.*[/:]([^/]+\/[^/]+?)(?:\.git)?$/, "$1"),
...
var FlowReporter = class {
  ...
  onBegin(_config, _suite) {
    ...
    this.screenshotsDir = path2.join(this.outputDir, "screenshots");
    fs2.mkdirSync(this.screenshotsDir, { recursive: true });
  ...
  onTestEnd(test2, result) {
    ...
    const screenshotAttachments = result.attachments.filter(
      (a) => a.name.startsWith("flow-screenshot-")
    );
    const screenshotIds = [];
    ...
    screenshotAttachments.forEach((attachment, index) => {
      ...
      const destPath = path2.join(this.screenshotsDir, fileName);
      if (attachment.path) {
        fs2.copyFileSync(attachment.path, destPath);
      } else if (attachment.body) {
        fs2.writeFileSync(destPath, attachment.body);
      }
    ...
    if (flowScreenshots.length === 0) {
      const builtinScreenshots = result.attachments.filter(
        (a) => a.contentType === "image/png" && !a.name.startsWith("flow-") && (a.path || a.body)
      );
    ...
    if (result.error) {
      ...
      if (trace && trace.length > 1e3) trace = trace.slice(0, 1e3) + "\u2026";
    }
    const steps = result.steps.filter((s) => s.category === "test.step").map((s) => ({
      name: s.title,
      status: s.error ? "failed" : "passed"
    }));
    const previousEntry = this.testMap.get(testId);
    const previousScreenshotFiles = [];
    if (previousEntry) {
      const prevAttachments = previousEntry.ctrfTest.attachments ?? [];
      for (const att of prevAttachments) {
        previousScreenshotFiles.push(path2.basename(att.path));
      }
    ...
    const ctrfTest = {
      ...
      message,
      trace,
      filePath: test2.location?.file,
      rawStatus: result.status,
      retries: retry,
      flaky: ctrfStatus === "passed" && retry > 0,
      steps: steps.length > 0 ? steps : void 0,
      attachments: ctrfAttachments.length > 0 ? ctrfAttachments : void 0,
      extra: flowScreenshots.length > 0 ? { flow: { screenshots: flowScreenshots, edges } } : void 0
    };
  ...
  async onEnd(_result) {
    ...
    for (const entry of Array.from(this.testMap.values())) {
      for (const fileName of entry.previousScreenshotFiles) {
        try {
          fs2.unlinkSync(path2.join(this.screenshotsDir, fileName));
        } catch {
        }
      }
    }
    ...
    const jsonPath = path2.join(this.outputDir, "ctrf-report.json");
    fs2.writeFileSync(jsonPath, JSON.stringify(report, null, 2), "utf-8");
    if (this.generateHtml) {
      try {
        const htmlPath = path2.join(this.outputDir, "index.html");
...
var reporter_default = FlowReporter;
...
var test = import_test.test.extend({
  page: async ({ page }, use, testInfo) => {
    ...
    const testId = slugify2(`${projectName}-${testInfo.title}`);
    const screenshots = [];
    let index = 0;
